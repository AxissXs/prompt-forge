...
import { sql, hashPassword, verifyPassword, newToken, verifyRecaptcha, getUserFromToken, json } from "./_lib";
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (action === "login") {
    ...
    return json({ token, user: { id: u.id, email: u.email, username: u.username, created_at: u.created_at } });
  }
